...
import com.xiami.core.AsciiUtil;
...
public class ECDictPanel extends JPanel implements Runnable,ActionListener{
	...
	private JMenuItem pmenuItem_lookUpEnglish = new JMenuItem();
	...
	public ECDictPanel(MainFrame obj, MainTabbedPane tabbedPane) {
		this.mainFrame = obj;
	...
	public void initDictPanel() {
		...
		if (!isInitialized) {
			...
			pMenu_Text.add(pmenuItem_lookUpEnglish);
			...
			pmenuItem_lookUpEnglish.addActionListener(this);
	...
	public void setting() {
		...
		keyWordBox.getInputContext().selectInputMethod(Locale.US);
	...
	public void setLanguage(){
		...
		pmenuItem_lookUpChinese.setText(SystemConfig.getLanguageProp("DictPanel.C2E"));
		pmenuItem_lookUpEnglish.setText(SystemConfig.getLanguageProp("DictPanel.E2C"));
		...
		TitledBorder fileBorder = new TitledBorder(SystemConfig.getLanguageProp("DictPanel.E2C"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == pmenuItem_lookUpEnglish) {
			String str = sentns.getSelectedText();
			this.setKeyWord(str);
...
//			System.out.println("takes "+(end-start));
	...
	public ArrayList<DictWordBean> getWordList(String keyWord){
		...
		try {
			beanList = loader.searchForE2C(keyWord);
		} catch (Exception e) {
	...
	public ArrayList<DictWordBean> getFutherWordList(String keyWord,int limit,boolean isNotFound){
		ArrayList<DictWordBean> beanList = new ArrayList<DictWordBean>();
		try {
			beanList = loader.futherSearchForE2C(keyWord,limit,isNotFound);
		} catch (Exception e) {
			JOptionPane.showMessageDialog(this, "Error was found when searching word "+keyWord);
		}
		return beanList;
	...
	public boolean filterWord(String keyWord) {
		...
		if (model_word.size() < MAX_SHOWED && keyWord.length() > 0) {
			futherFilterWord(keyWord, isNeedClear,MAX_SHOWED - model_word.size(),false);
		}
		if (model_word.size() > 0) {
		...
		for (int i = 0; i < bean.getSents().size(); i++) {
			if (wordList.get(index).getSents().get(i).startsWith(Keys.WORD_ELEMENT)
				...
				if (isMultiElement&&wordList.get(index).getSents().get(i).startsWith(Keys.WORD_ELEMENT)&&isStart) {
					...
					}else if (wordList.get(index).getSents().get(i)
...
//						str = wordList.get(index).getSents().get(i).replaceAll(Keys.SPLIT, "音节分段:") + "\n";
					...
					if(wordList.get(index).getSents().get(i).startsWith(Keys.HABI_PROTYPE)){
						...
						str = wordList.get(index).getSents().get(i).replaceAll(Keys.HABI_EXP, "惯用型解释:") + "\n";
						str="\n         " + str;
						sb.append(str);
						...
						if(wordList.get(index).getSents().get(i).equals(Keys.EXP)){
							if(isMultiExp){
								str = wordList.get(index).getSents().get(i).replaceAll(Keys.EXP, "★【"+exp_count+++"】:" +
										"【No English Explanation for this Item.】") ;
							}else{
		...
		String pron = "";
		if(null != bean.getPron()){
			pron=toPhontic(bean.getPron());
		}
...
//		keyWord.setText(bean.getWord());
	...
	public void run() {		
		try {
			initDictPanel();
			loader = new DictLoader(1);
		} catch (Exception e) {
			JOptionPane
			.showMessageDialog(getIns(),"Error found when initiating EC Dict.");
			e.printStackTrace();
		}
	}
	...
	public void updateLink(){
	 ...
	 public boolean isInputValid(String word) {
		...
		int hashCode = AsciiUtil.String2Ascii(firstWord.toLowerCase())[0];
		if (!(hashCode > 96 && hashCode < 123)
				&& !(hashCode > 12353 && hashCode < 12532)) {
			return false;
		}else{
			return true;
		}
	 ...
	 public String toPhontic(String word){
		 return word.replaceAll("!", "i").replaceAll("W\\*", "ɛ")
		 	.replaceAll("\\*i", "əri")
		 	.replaceAll("\\*\\*", "ərə").replaceAll("\\*", "ə")
			.replaceAll("6", "ˈ").replaceAll("7", "ˌ")
			.replaceAll("#", "æ").replaceAll("\\$", "α")
			.replaceAll("0", "ŋ").replaceAll("\\%", "ɔ")
			.replaceAll("t\\.", "tʃ")
			.replaceAll("\\(", "ʌ").replaceAll("d9", "dʒ")
			.replaceAll("8", ":").replaceAll("\\.", "ʃ")
			.replaceAll("G", "θ").replaceAll("H", "ð")
			.replaceAll("S", "ɴ").replaceAll("9", "ʒ")
			.replaceAll("]]", "]");
	 }
