...
public class SheetModifyFrame extends JDialog implements Runnable,
		ActionListener {
	...
	public SheetModifyFrame(WordPanel wordPanel) {
		type = 0;
		this.wordPanel = wordPanel;
	}

	public SheetModifyFrame(String name, WordPanel wordPanel) {
		type = 1;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == button_confirm) {
			if (text_name.getText().equals("")) {
				...
				this.dispose();
				if (type == 0) {
					wordPanel.addNewSheet(text_name.getText().trim());
				} else {
					wordPanel.updateSheetName(text_name.getText().trim());
				}				
			}
		}
	}

	public SheetModifyFrame getIns() {
		return this;
	}
}
