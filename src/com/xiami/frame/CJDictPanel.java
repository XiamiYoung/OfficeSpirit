...
public class CJDictPanel extends JPanel implements Runnable,ActionListener{
	...
	public CJDictPanel(MainFrame obj,MainTabbedPane tabbedPane) {
		this.mainFrame = obj;
	...
	public void setting() {
		...
		keyWord.getInputContext().selectInputMethod(Locale.CHINA);
	...
	public void setLanguage(){
		...
		TitledBorder fileBorder = new TitledBorder(SystemConfig.getLanguageProp("DictPanel.C2J"));
...
//			System.out.println("takes "+(end-start));
	...
	public ArrayList<DictWordBean> getWordList(String keyWord){
		...
		try {
			beanList = loader.searchForC2J(keyWord);
		} catch (Exception e) {
	...
	public ArrayList<DictWordBean> getFutherWordList(String keyWord,int limit,boolean isNotFound){
		ArrayList<DictWordBean> beanList = new ArrayList<DictWordBean>();
		try {
			beanList = loader.futherSearchForC2J(keyWord,limit,isNotFound);
		} catch (Exception e) {
			JOptionPane.showMessageDialog(this, "Error was found when searching word "+keyWord);
		}
		return beanList;
...
//		keyWord.setText(bean.getWord());
	...
	public void run() {		
		try {
			initDictPanel();
			loader = new DictLoader(4);
		} catch (Exception e) {
			JOptionPane
			.showMessageDialog(getIns(),"Error found when initiating CJ Dict.");
		}
	}
