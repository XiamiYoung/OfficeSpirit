...
public class CEDictPanel extends JPanel implements Runnable,ActionListener{
	...
	public CEDictPanel(MainFrame obj,MainTabbedPane tabbedPane) {
		this.mainFrame = obj;
	...
	public void setting() {
		...
		keyWordBox.getInputContext().selectInputMethod(Locale.CHINA);
	...
	public void setLanguage(){
		...
		TitledBorder fileBorder = new TitledBorder(SystemConfig.getLanguageProp("DictPanel.C2E"));
...
//			System.out.println("takes "+(end-start));
	...
	public ArrayList<DictWordBean> getWordList(String keyWord){
		...
		try {
			beanList = loader.searchForC2E(keyWord);
		} catch (Exception e) {
	...
	public ArrayList<DictWordBean> getFutherWordList(String keyWord,int limit,boolean isNotFound){
		ArrayList<DictWordBean> beanList = new ArrayList<DictWordBean>();
		try {
			beanList = loader.futherSearchForC2E(keyWord,limit,isNotFound);
		} catch (Exception e) {
			JOptionPane.showMessageDialog(this, "Error was found when searching word "+keyWord);
		}
		return beanList;
	...
	public void showWord(int index) {
		...
		boolean isHasProtype = false;
		boolean isExpAtFirstLine = true;
		String str = "";
			...
			if (wordList.get(index).getSents().get(i).startsWith(Keys.WORD_ELEMENT)
				...
				if (isMultiElement&&wordList.get(index).getSents().get(i).startsWith(Keys.WORD_ELEMENT)&&isStart) {
					...
					if(wordList.get(index).getSents().get(i).startsWith(Keys.PROTYPE)){
						...
						str += wordList.get(index).getSents().get(i).replaceAll(Keys.PROTYPE, "") + "\n";
					}else if (wordList.get(index).getSents().get(i)
							.startsWith(Keys.TYPE)) {
						str = wordList.get(index).getSents().get(i).replaceAll(
								Keys.TYPE, "词性:")
								+ "\n";
						isHasProtype = true;
...
//					}
						...
						str = wordList.get(index).getSents().get(i).replaceAll(Keys.SENTS_EXP, "☆例句:") + "\n";
						str="\n           " + str;
						sb.append(str);
						...
						if(isExpAtFirstLine&&!isHasProtype){
								isExpAtFirstLine = false;
							}else{
								str="\n" + str;
							}
							isExpAtFirstLine = false;
...
//		keyWord.setText(bean.getWord());
	...
	public void run() {		
		try {
			initDictPanel();
			loader = new DictLoader(2);
		} catch (Exception e) {
			JOptionPane
			.showMessageDialog(getIns(),"Error found when initiating CE Dict.");
		}
	}
